    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      ...
      {activeTab === 'PREMIUM' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {plans.map((plan) => (
                <div key={plan.name} className={`relative bg-slate-800 rounded-3xl p-8 border ${plan.recommended ? 'border-emerald-500 shadow-emerald-500/20 shadow-xl scale-105' : 'border-slate-700'} flex flex-col`}>
                    {plan.recommended && (
                        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-emerald-500 text-slate-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                            Most Popular
                        </div>
                    )}
            ...
            {groceryList.length === 0 ? (
                ...
                <ul className="space-y-3">
                    {groceryList.map((item, idx) => (
                        <li key={idx} className="flex items-center justify-between p-3 bg-slate-900/50 rounded-xl border border-slate-700 group">
      ...
      {activeTab === 'ITEMS' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {items.map((item) => (
                <div key={item.id} className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden group hover:border-emerald-500/50 transition-all">
                    <div className="h-40 bg-slate-700 flex items-center justify-center text-slate-500">
                        <Dumbbell size={40} />
                    </div>
                    <div className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button className="p-2 bg-emerald-500 text-slate-900 rounded-lg hover:bg-emerald-600 transition-colors">
                                <Plus size={20} />
                            </button>
                        </div>
                    </div>
